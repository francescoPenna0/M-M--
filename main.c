...
int main(int argc, char const *argv[])
{
    ...
    start1=clock(); // PARTE LA SIMULAZIONE
    for(utente.n_utenti=1;utente.n_utenti<input_utenti+1;utente.n_utenti++)
    {
        ...
        fprintf(ft,"P(k) definita dalla legge di poisson è : %.3f",nascita);
        ...
        fprintf(ft,"Il numero medio di pacchetti presenti nel sistema allo stato k è : %.3f\n",utente.lambda/utente.mu); // calcolo numero medio di pacchetti presenti nel sistema allo stato k
        fprintf(ft,"****************************************************\n");
        end=clock();  // TERMINA VITA DEL PACCHETTO

        double tempo = ((double)(end-start))/CLOCKS_PER_SEC; // calcolo la durata totale della vita del pacchetto al fine di calcolarne una media
	...
	printf("*    Tempo medio di permanenza in coda: Nessuno perchè è sempre disponibile un servitore\n");
